...
Tested with FM-507 reader. Will work with FM-503 and others in the same family.
...
| `U`<br>`U<slot q>`<br>`slot q`: number of Q-algrithm slots to receive tag responses                                                                                                                                                                                                                 | `U<none or EPC>`<br> none: no tag in RF field <br>EPC: PC+EPC+CRC16                                                                                                                                                                                          | Multi-TAG read EPC                                                                               |
...
| `QR: Q , R<bank>,<address>,<length>` <br>`<bank>` memory bank <br>`<address>` start word address 0 ~ 3FFF <br>`<length>` read word length 1 ~ 1E                                                    | `Q<EPC>,R<DATA>` or `<error code>`<br> EPC= PC+EPC+CRC16<br>DATA= read data                                                                                    | Multi-Band data read with EPC for single-Tag read                                                |
| `UR: U<slot Q or empty>, R<band>,<address>, <length>` <br>`<slot Q>`: 0~10 (hex) or '' ->EPC c1g2 Q-Algorithm slots to receive responses.<br>`<bank>` memory bank <br>`<address>` start word address 0 ~ 3FFF <br>`<length>` read word length 1 ~ 1E                              | `U<EPC>,R<DATA>` or `<error code>` <br>EPC= PC+EPC+CRC16 <br>DATA= read data                                                                                     | Multi-Band data read with EPC for multi-Tag read                                                 |
