...
| `UR: U<slot Q>, R<band>,<address>, <length>` <br>`<slot Q>`: 0~10 EPC c1g2 Q-Algorithm slots to receive answers.<br>`<bank>` memory bank <br>`<address>` start word address 0 ~ 3FFF <br>`<length>` read word length 1 ~ 1E                              | `U<EPC>,R<DATA>` or `<error code>` <br>EPC= PC+EPC+CRC16 <br>DATA= read data                                                                                     | Multi-Band data read with EPC for multi-Tag read                                                 |
